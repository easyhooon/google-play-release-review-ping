...
프로세스가 처음 시작될 때 API가 이미 오래된 `PUBLISHED` 릴리스를 반환할 수 있다. 기존 릴리스마다 게시 알림을 쏟아내지 않도록 첫 관측은 기준점으로만 저장한다.

그 결과 알리미를 심사가 끝난 뒤 처음 설치하면 현재 승인 상태에 대한 알림은 보내지 않는다. MVP는 오탐과 알림 폭주 방지를 우선하며, 현재 상태 즉시 알림 옵션은 후속 범위로 둔다.
